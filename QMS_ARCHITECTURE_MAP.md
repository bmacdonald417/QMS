...
- **Flow (actual):**
  ...
  - **IN_REVIEW** → Reviewers must complete a **reviewer questionnaire** (formatting, incomplete items, and up to 5 other questions). They POST review with `decision` and `reviewResponses`. If any answer indicates corrections needed, the only allowed decision is REQUIRES_REVISION: document returns to **DRAFT**, author is notified, and **draft round** is incremented on the document. If REQUIRES_REVISION → back to **DRAFT** (author sees “Draft (Round N)” and history shows which questions were flagged). If all reviews completed with no corrections required → approver is notified; approver POSTs approve (with password for e-sign) → **APPROVED** (DocumentSignature created).
