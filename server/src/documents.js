...
// POST /api/documents/:id/sign — Add a digital signature (Prepared By, Reviewed By, Approved By, etc.) with password, audit log, and hashes
router.post(
  '/:id/sign',
  requirePermission('document:view'),
  async (req, res) => {
    try {
      const { signatureMeaning, password, comments } = req.body;
      if (!signatureMeaning || typeof signatureMeaning !== 'string' || !signatureMeaning.trim()) {
        return res.status(400).json({ error: 'signatureMeaning is required' });
      }
      if (!password) {
      ...
      const document = await prisma.document.findUnique({
        where: { id: req.params.id },
        select: { id: true, content: true, documentId: true, versionMajor: true, versionMinor: true },
      });
      if (!document) return res.status(404).json({ error: 'Document not found' });
      ...
      const meaning = signatureMeaning.trim();
      ...
      const created = await prisma.documentSignature.findFirst({
        where: { documentId: document.id, signerId: req.user.id },
        orderBy: { signedAt: 'desc' },
        include: { signer: { select: { id: true, firstName: true, lastName: true } } },
      });
      res.status(201).json({
        signature: {
          id: created.id,
          signatureMeaning: created.signatureMeaning,
      ...
      res.status(500).json({ error: err.message || 'Failed to add signature' });
    }
  }
);
