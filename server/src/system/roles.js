    ...
    try {
      ...
      res.json({
        roles: roles.map((r) => ({
          ...
          permissionDetails: r.rolePermissions?.map((rp) => rp.permission?.code) || [],
        })),
        permissions,
      });
    } catch (err) {
...
// GET /api/system/roles/permission-matrix
router.get(
  '/permission-matrix',
  requireSystemRole('System Admin', 'Admin', 'Quality Admin', 'Quality Manager'),
  requireSystemPermission('auditlog:view'),
    ...
    try {
      ...
      const matrix = roles.map((role) => ({
        ...
        permissions: permissions.map((p) => ({
          permissionId: p.id,
          code: p.code,
          granted: role.permissions?.includes(p.code) || set.has(`${role.id}-${p.id}`),
        })),
      }));
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (existing.name === SYSTEM_ADMIN && body.permissions && !body.permissions.includes('*')) {
